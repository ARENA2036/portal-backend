...
public class BpdmBusinessLogicTests
{
    ...
    public async Task HandlePullLegalEntity_BringYourOwnWallet_True_ReturnsExpected()
    {
        ...
        var bpdmLegalEntityOutputData = _fixture.Build<BpdmLegalEntityOutputData>()
            .With(x => x.LegalEntity,
                new BpdmLegelEntityData("CAXSDUMMYARENAXZZ",
                null,
                null,
                null,
    ...
    private void SetupForHandlePullLegalEntity(Company? company = null)
    {
        ...
        A.CallTo(() => _bpdmService.FetchInputLegalEntity(A<string>.That.Matches(x => x == IdWithBpn.ToString()), A<CancellationToken>._))
            .Returns(_fixture.Build<BpdmLegalEntityOutputData>().With(x => x.LegalEntity, new BpdmLegelEntityData("CAXSDUMMYARENAXZZ", null, null, null, _fixture.Create<BpdmConfidenceCriteria>(), Enumerable.Empty<BpdmStatus>())).Create());
